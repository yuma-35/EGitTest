...
import Displays.Account.toStartS;
 ...
 toSounds.setForeground(Color.WHITE);
	...
	void reloadMatch() {
		...
		/*
	     ...
	     MatchPanel.add(new Match(i,read));
	...
	void reloadFriendlist() {
		/*aいくつかのプレイヤクラス&卓クラスのセットが飛んでくる
		 int friendNumber=friendの数
		  int statusBox
		  Player PlayerBox;
		   Match  MatchBox; 
		  statusBox=status
		...
		friendlistPanel.setPreferredSize(new Dimension(160,80*friendnumber));
		for(int i;i<friendnumber;i++){
	...
	void reloadMainmenu() {
		...
		this.rankpointLabel.setText("あと"+Client.myPlayer.rankPoint+"pt");
	reloadFriendlist();
	reloadMatch();
	}
	...
	public class FriendPanel extends JPanel{
		 ...
		 public class friendBattleB implements ActionListener{
		     	public  void actionPerformed(ActionEvent e) {
		    if(statusBox==1) {//申込
